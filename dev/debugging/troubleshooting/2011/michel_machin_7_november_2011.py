def
importSpkTrain(Nout,file_name="embeddedInputs.mat",var="embeddedInputs"):
...
spkTimeTemp.append(Quantity(inputSpkTimes[j,inputInstance-1][ind1]
[0])*ms)

        allSpkTrain.append(spkTimeTemp)

    shuffleNeur=random.permutation(Nout)
...
def importDelays(file_name2="delta_ij.mat",var2="delta_ij"):
    global delta_ij
    d = scipy.io.loadmat(file_name2, matlab_compatible=True,
mdict=None)
    delta_ij=d[var2]
...
C_m = 0.5 * nF     # capacitance
Vt = -50 * mV        # spike threshold
Vr = -65 * mV        # resting potential and reset value
stdVr = 3               # std of Vr
E_L = -70 * mV     # resting potential
gL = 100 * nS        # leak conductance 10
...
# COBA model
eqs = Equations('''
        dV/dt = (-gL*(V-E_L)-g_ex*(V-E_ex)-g_inh*(V-E_inh))*(1./
C_m)    : volt
        dg_ex/dt = -g_ex/
tau_ex                                                                 :
nS
...
OutLayer = NeuronGroup(N=Nout,  model=eqs, threshold=Vt, reset=Vr,
refractory=ref, clock=simclock, freeze=False, implicit=True)
...
# connections
C_ex =
Connection(Input_ex,OutLayer,'g_ex',delay=True,max_delay=delta_ij.max()*ms)
C_inh =
Connection(Input_inh,OutLayer,'g_inh',delay=True,max_delay=delta_ij.max()*ms)

for ej,ei in ((ej,ei) for ej in range(len(ExNeur)) for ei in
range(Nout)):
    C_ex.delay[ej,ei] =
Quantity(delta_ij[ExNeur[ej],inputInstance-1])*ms

C_ex.connect(Input_ex,OutLayer,W_ex)

for ij,ii in ((ij,ii) for ij in range(len(InhNeur)) for ii in
range(Nout)):
    C_inh.delay[ij,ii] =
Quantity(delta_ij[InhNeur[ij],inputInstance-1])*ms
...
OutLayer.V = Vr+(Vt-Vr)*rand(len(OutLayer)) # uniform random
initialization of the  membrane voltage V of the Output layer between
Vr and Vt
