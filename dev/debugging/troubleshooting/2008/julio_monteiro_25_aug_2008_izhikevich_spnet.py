...
if os.path.exists('izhikevich.pkl'):
    print "Loading saved network from file..."
    inputfile = open('izhikevich.pkl','rb')
    objs = pickle.load(inputfile)
    inputfile.close()
    clk, model, G, Ce, Cie, S, net = objs
...
def do_run(T=T,doplot=True):
    start_time = time.time()
    print "Running the Izhikevich spnet simulation for %s:" % display_in_unit(T,second)
    net.run(T)
...
clk = defaultclock
obj = (clk, model, G, Ce, Cie, S, net)
output = open('izhikevich.pkl', 'wb')
