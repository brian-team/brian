...
class AcquisitionBoard(NeuronGroup):
    ...
    def __init__(self,P,record,command,clock=None):
        ...
        ''',units_record=P.unit(record),units_command=P.unit(command))
    ...
    def __call__(self):
        self.record=self._group.state(self._record) # Record
        self._group.state(self._command)[:]=self.command # Inject
...
class DCC(AcquisitionBoard):
    ...
    def __call__(self):
        if self.cycle==0:
            self.record=self._group.state(self._record) # Record
            self._group.state(self._command)[:]=3*self.command # Inject
        else:
            self._group.state(self._command)[:]=0 #*nA
