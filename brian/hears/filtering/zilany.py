...
class Chirp_Coefficients:
    ...
    def __init__(self,cf,taumax,samplerate,rsigma,fcohc):
        ...
        self.preal = zeros((self.nch,10))
        self.pimg = zeros((self.nch,10))
        ...
        for ich in xrange(self.nch):
            self.Cinitphase[ich] = sum(arctan(self.CF[ich]/(-self.rzero[ich]))\
    ...
    #        print norm_gain
            self.phase[ich] = sum(-arctan((self.CF[ich]-self.pimg[ich,[0,2,4,6,8]])/(-self.preal[ich,[0,2,4,6,8]]))\
                                -arctan((self.CF[ich]+self.pimg[ich,[0,2,4,6,8]])/(-self.preal[ich,[0,2,4,6,8]])))
            self.rzero[ich] = -self.CF[ich]/tan((self.Cinitphase[ich]-self.phase[ich])/self.order_of_zero)
            for ind,iord in enumerate([0,2,4,6,8]):
            
                temp = (self.fs_bilinear[ich]-self.preal[ich,iord])**2+self.pimg[ich,iord]**2
                
                self.filt_a[ich,:,ind] = array([1,(-2*(self.fs_bilinear[ich]**2-self.preal[ich,iord]**2-self.pimg[ich,iord]**2))/temp,\
                                                ((self.fs_bilinear[ich]+self.preal[ich,iord])**2+self.pimg[ich,iord]**2)/temp])
...
#        print self.filt_a.flags,self.filt_b.flags
    ...
    def analog_poles(self,rsigma,fcohc):
        for ich in xrange(self.nch):
#            print fcohc,rsigma
            self.preal[ich,0] = -self.sigma0[ich]*fcohc[ich]-rsigma[ich]
            self.preal[ich,4] = self.preal[ich,0] - self.rpa[ich]
            self.preal[ich,2] = (self.preal[ich,0]+self.preal[ich,4])*0.5
            self.preal[ich,1] =  self.preal[ich,0]
            self.preal[ich,3] =  self.preal[ich,2]
            self.preal[ich,5] =  self.preal[ich,4]
            self.preal[ich,6] =  self.preal[ich,0]
            self.preal[ich,7] =  self.preal[ich,1]
            ...
            self.pimg[ich,1] = -self.pimg[ich,0]
            self.pimg[ich,3] = -self.pimg[ich,2]
            self.pimg[ich,5] = -self.pimg[ich,4]
            self.pimg[ich,6] =  self.pimg[ich,0]
            self.pimg[ich,7] =  self.pimg[ich,1]
            self.pimg[ich,8] =  self.pimg[ich,4]
            self.pimg[ich,9] =  self.pimg[ich,5]
...
#        ##### Control Path ####
        print centerfreq
#        gt_control = ApproximateGammatone(source, centerfreq, 1./(tauwb), order=3) 
...
#            return out
#        print (tauwb/TauWBMax)**wborder*10e3*maximum(ones(len(cf)),cf/5e3)
#        gt_control1 = FunctionFilterbank(gt_control,wb_gain,cf=cf,tauwb=tauwb,TauWBMax=TauWBMax,wborder=wborder) 

#        gt_control = Gammatone(source, centerfreq, b=1./(1*pi*tauwb))
        # first non linearity of control path
        NL1_control=FunctionFilterbank(wbout,boltzman,asym=ohcasym,s0=12.0,s1=5.0,x1=5.0)        
