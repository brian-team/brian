...
SoundDuration=500*ms    #duration of the whole sequence
sound=whitenoise(SoundDuration).ramp() 
...
def vary_filter_init(fs,coeff,m_i,s_i,tau_i):
    N=len(m_i)
    b=zeros((N, 3, 1))
    a=zeros((N, 3, 1))
    fc=m_i
    ...
    fc=fc-fc/tau_i*deltaT+mu_i*deltaT+sigma_i*randn(1)*sqrt(deltaT)
    BWhz=fc/Q
...
#        fc=fs/2-1000*Hz
    ...
    a[:, 1, 0]=-2*cos(w0)
    a[:, 2, 0]=1-alpha
    return b,a,[fc,mu_i,tau_i,deltaT,sigma_i,BW,Q]
      
def vary_filter(fs,N,fc,mu_i,tau_i,deltaT,sigma_i,BW,Q):
...
fb2= TimeVaryingIIRFilterbank2(sound.rate,nbr_channel,vary_filter_init,vary_filter,coeff*ones(nbr_channel),m_i*ones(nbr_channel),s_i*ones(nbr_channel),tau_i*ones(nbr_channel))
